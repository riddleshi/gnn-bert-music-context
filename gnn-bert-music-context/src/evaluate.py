...
def build_demo_payload(cfg, loaders, models, fusion_blob, report, retrieval_rows, bert_examples, vocab, device) -> dict[str, Any]:
    ...
    case_ids = []
    for want in ("folk", "electronic", "hip-hop", "rock", "classical", "pop", "jazz", "metal"):
        found = next((tid for tid in test_ids if by_id[tid]["genre"] == want), None)
        if found and found not in case_ids:
    ...
    return {
        ...
        "corpus": {
            "n_tracks": len(tracks),
            "source": "fma" if any(str(t["id"]).startswith("fma_") for t in tracks) else "synthetic",
        },
    }
