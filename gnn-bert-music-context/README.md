...
python scripts/build_corpus.py          # synthetic FMA+MusicCaps+DEAM stand-in
...
Official dumps (FMA-small/medium, MagnaTagATune, MusicCaps, DEAM, GTZAN) are large and are **not** vendored. `src/synthetic.py` builds a 288-track artist-split corpus that is genre-conditioned in harmony, tags, captions, and valence/arousal so every script runs offline.

When you have real audio:

1. Place files under `data/raw/` (see `src/real_data.py` for the expected layout).
2. Reuse `src.audio_features.extract_bundle` + `src.graph_builder.build_graphs`.
3. Emit records with the same keys as the cached corpus (`seg_x`, `seg_adj`, `caption`, `tags`, …). `src.train` does not change.
...
NeurIPS-style write-up: [`report/final_report.pdf`](report/final_report.pdf) (source: [`report/final_report.md`](report/final_report.md), LaTeX: [`report/final_report.tex`](report/final_report.tex)). Rebuild with `pdflatex -output-directory report report/final_report.tex`.
